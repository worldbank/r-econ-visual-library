...
This graph uses fictional data created for the purpose of this graph. The fictional data comes from an imagined survey of tropical farmers participating in a fictional randomized control trial. 
...
reshaped_data <- collapsed_data %>% # Reshape crop variables to long format
                  pivot_longer(starts_with(c("w_")), # Select crop variables
...
# Printing the first five rows of the data to visualize its structure after reshaping
head(collapsed_data)
...
ggplot(reshaped_data, aes(x = key, y = value, fill = round)) +
  ...
  scale_x_discrete(
    labels = c("w_Papaya" = "Papaya Prod", "w_Pineapple" = "Pineapple Prod",
