...
df_coef_logit <- df_result %>%
  ...
  mutate(
    # Choose appropriate critical vaulue for lower and upper bounds
    lower_bound = exp(estimate - 1.96 * std.error),
    upper_bound = exp(estimate + 1.96 * std.error),
    OR = exp(estimate), # exp is used for euler's number 
...
p <- ggplot(df_coef_logit, aes(x = y_var, y = OR)) + 
  ...
  theme_classic() +
  # TeX is used to display characters in LaTeX
...
# Using the ggplot object p with annotate function to add extra labels
p + annotate(
